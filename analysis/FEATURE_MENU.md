...
由 `analysis/42_features_full.py` 生成 → `analysis/features.csv`(24 人 × 351 列)。
...
## 注意(n=24 的方法学后果)
351 特征 ≫ 24 样本 → **必然有碰巧相关的**。阶段3/4 用留一 CV(fold 内做特征选择/标准化)+ 多重比较校正(FDR/置换)来防"把噪声当发现"。此表是"全做出来"的原料,不是结论。
