...
| `analysis/53_stat_budget_probes.py` | 统计预算探针×2：①特征数增长对 BH-FDR 校正的代价——往现有 p 值里掺 K 个纯噪声特征重算 q，含"新增里有 1 个真信号"的反向情形（供 TASK-10/TASK-108/ISSUE-121）②n=24 的检验效能与置信区间——最小可测 rho、power、CI 宽度、达 80% power 所需样本量（供 ISSUE-116）③置换分辨率给 FDR 设的硬门槛——两轨 p 下限公式不同（A 轨 `1/NPERM`、B 轨 `1/(NPERM+1)`，NPERM 从生产脚本正则解析、不硬编），算各家族口径下「需多少个检验并列达到 p 下限才可能有一条通过 q<0.05」（供 ISSUE-121 / TASK-113）| L0 探针 | `analysis/A_univariate.csv`（探针2 不读任何数据，纯统计公式；`ADHD_ROOT` 可覆盖）| 仅 print（不写盘）| 活跃（裁决时段决策支持）〔实证〕 |
| `analysis/54_duration_confound_probe.py` | 混淆探针：截等长（TASK-102）之后，「原始录制时长」是否仍与症状分、与 351 个特征两头都相关——即它是否仍是混淆变量。只数 CSV 行数取点数、不解析内容，Spearman 口径与 A 轨一致。给 ISSUE-103 的「跨子量表对比配不配时长偏相关」与 ISSUE-114 供数，结论落在 `backlog.md` §9 的 R11 | L0 探针 | `data/{s}_T.csv`（仅 `wc -l`），`figures/subject_audit.csv`，`analysis/targets.csv`，`analysis/features.csv`（`ADHD_ROOT` 可覆盖）| 仅 print（不写盘）| 活跃（ISSUE-103/114 决策支持）〔实证〕 |
| `consistency_explained.py`（根目录）〔**已归档**〔2026-07-28，TASK-5 动作 D〕：现位于 **`archive/consistency_explained.py`**，内容未改（10239 字节）〕 | 教学版：逐行讲解 20_codebook_verify 的 SDQ 一致性段 | L0 探针 | `data/Demographic and mental health data.csv` | 仅 print | 废弃（教学副本，逻辑与 20 的 C4 段重复）|
| `analysis/65_oof_predictions.py` | 保存 B 轨每个组合的 24 个**留出预测**（out-of-fold predictions）。**独立于 `45_multivariate_cv.py`**：不碰它、也不碰 `B_multivariate.csv`（那张表背后是 23.7 小时的置换）。管线与模型定义**不手抄**，用 `ast` 从 `45_multivariate_cv.py` 原样抽出 `reg_pipe`/`clf_pipe`/`reg_skill`/`REG`/`CLF`/`KS` 执行，故 45 改了本脚本跟着改、不会各自漂移（同款先例：`analysis/probe_outputs/perm_checkpoint_test.py`）。**自带对账**：算完反推 7 个指标与 `B_multivariate.csv` 逐格比对，2026-08-05 实测 612/612 组合全部对上、最大偏差 1.78e-15（浮点舍入）。实测耗时 2 分 57 秒 | L3 建模 | `analysis/features.csv`,`targets.csv`,`target_labels.csv`,`target_labels_meta.csv`,`analysis/45_multivariate_cv.py`（抽定义）,`data/Demographic and mental health data.csv`（仅两个 BMI 探索臂需要；取不到就只算 main 臂并明确打印）| `analysis/B_oof_predictions.csv` | **活跃**〔实证〕 |
...
| `analysis/A_univariate.csv` | A 轨结果（5775 行）〔**现为 12,160 数据行 × 11 列**，2026-07-30 实测订正；旧数产于 275 列特征表那版〕| L3 产物 | 44 | 无脚本读〔**不成立**，2026-07-28 查出：`analysis/53_stat_budget_probes.py` 读它（`A_PATH = ROOT / "analysis/A_univariate.csv"` 那行赋值、5 行后 `pd.read_csv(A_PATH)`），**只用 `perm_p` 一列**〕| 最终产物〔实证〕**〔2026-07-29 · TASK-106：已重跑并入版本控制**（608 列特征表 + 置换 100000 次），落点 commit `c0cc6ff`；**2026-07-30 · TASK-120：`rho` 列拆成 `rho`（连续行）+ `auc`（二分行）两列**，数值一格未变（逐格比对 17 项全通过），落点见该条；**2026-07-30 同日 · TASK-121：`rho_partial_uamag` 同样拆成 `rho_partial_uamag`（连续行）+ `auc_partial_uamag`（二分行）**，数值同样一格未变（逐格比对 17 项全通过）。**故本表现为 12,160 数据行 × 12 列**〕 |
| `analysis/B_oof_predictions.csv` | B 轨每个组合的**留出预测**长表（一行 = 一个「组合 × 被试」）：`variant, track, target, model, k, subject, y_true, y_pred`。2026-08-05 实测 **14,280 行 × 8 列**、0.69 MB = 612 个组合 ×（main 24 人 / 两个 n=23 臂各 23 人）。**用途**：`B_multivariate.csv` 只有汇总指标，逐人预测在那里查不到；本表补上残差图、预测-真值散点、混淆矩阵、逐人误差所需的原料。**入库理由见 `CLAUDE.md`「产物入库的判据」的第二处例外**（它不满足「被下游读」与「重生成有门槛」）| L3 产物 | 65 | 无脚本读 | 最终产物〔实证〕 |
...
- **`consistency_explained.py`**：教学副本，逻辑与 `20_codebook_verify.py` 的 C4/一致性段重复；无人读、不产文件。**孤儿**。〔**已归档**〔2026-07-28，TASK-5 动作 D〕：移动前重新实测确认全仓库无任何 `.py` / notebook 引用（只有自引与文档描述），现位于 **`archive/consistency_explained.py`**〕
