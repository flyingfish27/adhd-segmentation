...
数字前缀按**功能族**分段。此前该约定只活在惯例里、从未被任何文件定义过（2026-07-26 补写）。
...
| `20`–`29` | 码本核验 + 探索性分析 |
| `30`–`36` | 论文复现 + 常模核查 |
| `40`–`48` | **主管线**（targets / features / labels / A 轨 / B 轨）+ 审计与验收 |
| `50`–` ` | **探针**（为某条任务或 issue 供数的一次性只读脚本） |

**四条规矩**（2026-07-26 用户裁决，起因是并行开桌时两张桌各自取了 `55_` 与 `56_`、因文件名不同而 git 不报冲突、属静默撞号）：
1. **一号一主题。** 同一主题下的多个脚本用**字母紧跟数字**展开，如 `55a_xxx.py` / `55b_xxx.py`——排序天然正确（`55a < 55b < 55c < 56`），且永不溢出号段。
2. **既有的 `50`–`54` 不动**（它们本来就一号一主题，改名只会让引用它们的文档再漂一次）。
3. **并行开桌前，由管理窗口预分配号段。**
4. **agent 不得自行取号**——要用新号必须先问用户。

**为什么第 3、4 条是必需的**：撞号的根因**不是号不够用，是并行的两桌互相不知道对方在用什么号**；而 git 对"文件名不同的同号"**不报冲突**，所以撞了也不会被发现，只能靠人事后看目录列表。加字母只解决扩展性，不解决并发不知情。
