...
### 3.1 中国人群阈值:外部常模检索(见 `analysis/chinese_norms.md`)

论文1 的 **T≥55 是拿其自身 50 人算的样本内 z 分,非中国人群常模,也非任何验证过的临床切点**。外部文献检索(2026-07-16,关键来源已亲自 WebFetch 复核)结论:

| 量表 | 中国阈值现状 | 可用来源(依据) |
|---|---|---|
| **SDQ 多动子量表** | **有大陆常模**:标准 0–10 计分下 **异常≥8**(正常 0–6/边界 7) | 大陆 22,108 人家长版(PMC4054577);上海 n=1,965(PMC2409296) — 复现/文献 |
| **SDQ 总困难** | 大陆:异常 ≥19(8省样本)或 ≥17(上海) | 同上 |
| **SNAP-IV** | **无人群百分位/T 分常模**;仅有 DSM 式**症状计数** caseness(注意力/多动 ≥7 题评分≥2,ODD ≥5 题) | 大陆临床 n=327(PMC10326278, Huang 2023) — 文献 |

> **应用换算**:本数据 1-indexed(CODEBOOK §1)。SDQ 套 0–10 切点须每题**减 1**;SNAP "评分≥2" 对应**本数据评分≥3**。
> **误杀记录**:检索的对抗验证曾将 Huang 2023 的 SNAP 症状计数规则判为"驳倒"(1-2),经亲自读原文确认**成立**(假阴性),以原文为准。
...
| 相邻采样点大量完全相等(疑似重采样/采样保持痕迹) | 被试 **H45** 的 30Hz `_T` 文件里，相邻两个采样点的"去重力动作强度"之差**恰好为 0** 的比例达 **43.75%(46,589 / 106,488)**。这是量化台阶或采样保持(sample-hold)的典型痕迹，与 ISSUE-109 所疑的"`_T` 文件可能是被重采样/处理过的、不是原始数据"方向一致。⚠️ **仅测过 H45 一人**，产出脚本为临时探针、未入库，要引用须先在 24 人上重算 | TASK-1 会话 2026-07-25 临时探针(脚本已失)，登记于此 |
